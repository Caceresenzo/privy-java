...
public interface FeignPrivyClient {
	...
	@RequestLine("GET /v1/users?limit={limit}&cursor={cursor}")
	...
	@RequestLine("POST /v1/users/email/address")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByEmail(AddressRequest body);

	@RequestLine("POST /v1/users/wallet/address")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByWallet(AddressRequest body);

	@RequestLine("POST /v1/users/phone/number")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByPhone(PhoneRequest body);

	@RequestLine("POST /v1/users/twitter/username")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByTwitterUsername(UsernameRequest body);

	@RequestLine("POST /v1/users/twitter/subject")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByTwitterSubject(SubjectRequest body);

	@RequestLine("POST /v1/users/discord/username")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByDiscordUsername(UsernameRequest body);

	@RequestLine("POST /v1/users/github/username")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByGithubUsername(UsernameRequest body);

	@RequestLine("POST /v1/users/custom_auth/id")
	@Headers(JSON_CONTENT_TYPE)
	User getUserByCustomAuthId(CustomUserIdRequest body);

	@RequestLine("POST /v1/users/{id}/custom_metadata")
	@Headers(JSON_CONTENT_TYPE)
